...
class TranscriptCVParser:
    ...
    def _extract_projects(self, text: str) -> List[Dict[str, Any]]:
        ...
        # FIXED: Better project splitting - capture ALL projects with more flexible patterns
        # Look for project markers: "first project", "second project", "second project name is", etc.
        project_markers = [
        ...
        project_positions = sorted(list(set(project_positions)))
        ...
        for i, start_pos in enumerate(project_positions):
            ...
            # Extract project name - more flexible pattern
            name_patterns = [
                r"project(?:\s+name)?\s+is\s+([^.,]+?)(?:\s+and\s+client|\.|,|\s+client)",
                r"project(?:\s+name)?\s+is\s+([^.]+?)(?:\.|client\s+is)",
            ]
            ...
            for pattern in name_patterns:
                ...
                if name_match:
                    ...
                    project["project_name"] = name.title()
                    break
            
            # FIXED: Extract client - better pattern with more flexibility
            client_patterns = [
                r"client\s+is\s+([^.,]+?)(?:\.|,|\s+project\s+description)",
                r"(?:and\s+)?client\s+is\s+([^.,\n]+?)(?:\.|,|$|\s+project)",
            ]
            
            for pattern in client_patterns:
                client_match = re.search(pattern, section, re.IGNORECASE)
                if client_match:
                    client = client_match.group(1).strip()
                    # Remove trailing words
                    client = re.sub(r'\s+(?:and|the)$', '', client, flags=re.IGNORECASE)
                    if client and len(client) > 2:
            ...
            desc_patterns = [
                ...
                r"(?:client\s+is\s+[^.]+?\.\s*)project\s+description\s+(?:here\s+)?is\s+(.+?)(?:coming\s+to|roles?\s+and)",
            ]
            ...
            for pattern in desc_patterns:
                ...
                if desc_match:
                    ...
                    description = re.sub(r'^is\s+', '', description, flags=re.IGNORECASE)
                    
                    # Only use if it's a real description
                    if len(description) > 20 and 'my role' not in description.lower():
    ...
    def _extract_education(self, text: str) -> List[Dict[str, Any]]:
        """Extract education details - COMPLETELY REWRITTEN"""
        education = []
        
        # Look for educational qualifications section - more flexible
        edu_patterns = [
            r"(?:coming\s+to\s+)?(?:my\s+)?educational\s+(?:details|qualifications?)(.+?)(?:thank\s+you|that'?s?\s+it|$)",
            r"(?:coming\s+to\s+)?(?:my\s+)?education(.+?)(?:thank\s+you|that'?s?\s+it|$)",
        ]
        ...
        # FIXED: Extract 12th standard (Intermediate)
        twelfth_patterns = [
            r"(?:completed\s+)?intermediate.*?(?:that\s+is\s+)?12th\s+standard.*?branch\s+is\s+([^.,]+)[.,].*?college.*?is\s+([^.,]+)[.,].*?(?:university.*?is|board).*?([^.,]+?)[.,].*?(?:percentage|i\s+got).*?(\d+)",
            r"12th.*?branch[:\s]+([A-Z]+).*?college[:\s]+([^.,]+).*?board[:\s]+([^.,]+).*?(\d+)\s*percentile",
        ]
